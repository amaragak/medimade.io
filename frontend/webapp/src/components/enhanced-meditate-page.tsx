...
function StartFreeButton({ className = "" }: { className?: string }) {
  return (
    <Link
      href="/meditate/create"
      className={`inline-flex items-center justify-center rounded-full accent-fill-gradient px-7 py-3 text-sm font-semibold text-on-accent transition-opacity hover:opacity-90 ${className}`}
    >
      Start free
    </Link>
  );
...
export function EnhancedMeditatePage() {
  return (
    <div className="w-full">
      <section className="home-hero home-hero--meditate w-full px-4 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-16">
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-2 lg:gap-14">
          ...
          <div className="rounded-2xl border border-marketing-card-border bg-marketing-panel-bg p-5 sm:p-6">
            ...
            <Link
              ...
              className="mt-5 inline-flex items-center rounded-full accent-fill-gradient px-5 py-2.5 text-sm font-semibold text-on-accent transition-opacity hover:opacity-90"
            >
