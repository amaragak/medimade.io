...
export function scriptLabSegmentDurationBudgetAppendix(params: {
  ...
  return [
    ...
    "",
    "For each `{ tag: \"TAG_NAME\" }` beat, add that tag's **average word count** from the library listing above:",
    ...
    "",
    "For each `{ beatType: \"…\", text: \"…\" }` custom beat, count the words in `text` directly (ignore `[[PAUSE …]]` markers inside custom text — those add silence separately).",
    "",
