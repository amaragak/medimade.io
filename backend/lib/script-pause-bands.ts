...
export const SCRIPT_PAUSE_PROMPT_RULES = [
  ...
  "Choose the **band** by context: **short** when momentum matters; **medium** as the typical gap between lines; **long** after heavier invitations, imagery, or emotional lines; **extra long** when the listener is practising in their own time (breath or body at their own pace, counting, slow body scan, open visualization, resting in silence). Default toward more frequent silence than a dense script—still never gratuitous.",
  "When the listener follows in their own time, prefer **extra long** (sometimes several markers in a row when one sustained silence fits); never rush the next line while they are meant to be practising alone, and never stack extra-long silence where the script does not call for it.",
